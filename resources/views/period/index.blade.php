    ...
    <x-slot name="header">
        <div class="flex">
            ...
            <div class="w-1/2 text-right">
                <a href="{{url('/period/create')}}" class="text-white px-5 py-3 bg-gray-600">
                    Create New
                </a>
            </div>
        </div>
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <table class="min-w-max w-full table-auto table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="py-5">{{$period->id}}</td>
                                <td class="py-5">{{$period->month}}</td>
                                <td class="py-5">{{$period->year}}</td>
                                <td class="py-5">{{$period->description}}</td>
                                <td class="py-5">
                                    <div class="flex">
                                        <a href="{{route('period.edit',['period'=>$period->id])}}">
                                        ...
                                        <form method="POST" action="{{route('period.destroy',['period'=>$period->id])}}">
                                            @csrf
                                            @method('delete')
                                            <button type="submit">
